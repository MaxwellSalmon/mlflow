...
class ModelConfig:
    ...
    which would be used as configuration for the model. But this development_config configuration
    file will be overridden when logging a model. When no model_config is passed in while logging
    the model, an error will be raised when trying to load the model using ModelConfig.
    ...
    .. code-block:: python
        ...
        loaded_model = mlflow.pyfunc.load_model(model_uri="runs:/" + run.info.run_id + "/model")
